...
public class Main {
    ...
    //Tast1
        List<Animal> sortedListByHeight = list.stream().sorted(Comparator.comparingInt(Animal::height)).collect(Collectors.toList());
        sortedListByHeight.forEach(System.out::println);
        System.out.println();

    //Task2
        int k = 2;
        List<Animal> sortedListByWeight = list.stream().sorted(Comparator.comparingInt(Animal::weight).reversed()).limit(k).collect(Collectors.toList());
    ...
    //Task7
        k = 1;
        Animal maxKAge = list.stream().sorted(Comparator.comparing(Animal::age).reversed()).collect(Collectors.toList()).get(k - 1);
    ...
    //Task9
        Integer pawsNum = list.stream().collect(Collectors.summingInt(Animal::paws));
        System.out.println(pawsNum + "\n");

    //Task10
        List<Animal> listAnimalsWithNotEqualsAgeAndPaws = list.stream().filter(x-> x.paws() != x.age()).collect(Collectors.toList());
        listAnimalsWithNotEqualsAgeAndPaws.forEach(System.out::println);
        System.out.println();

    //Task11
        List<Animal> listBitedAnimals = list.stream().filter(x-> (x.bites() && x.height() >= 100)).collect(Collectors.toList());
        listBitedAnimals.forEach(System.out::println);
        System.out.println();

    //Task12
        Long numAnimalsWeightMoreHeight = list.stream().filter(x -> x.weight() > x.height()).collect(Collectors.counting());
        System.out.println(numAnimalsWeightMoreHeight + "\n");

    //Task13
        List<Animal> listAnimalsWithPluralNames = list.stream().filter(x-> (x.name().split(" ").length > 1)).collect(Collectors.toList());
